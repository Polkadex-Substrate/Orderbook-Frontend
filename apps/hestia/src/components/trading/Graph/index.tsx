...
const GraphV1 = ({ currentMarket }: { currentMarket?: Market }) => {
  ...
  const [statusBadge, setStatusBadge] = useState({
    ...
    className:
      "px-3 py-1 rounded-full text-xs font-medium bg-blue-900 text-blue-300",
  });
    ...
    const initChart = () => {
      ...
      chart.subscribeCrosshairMove((param) => {
        if (param.time && param.seriesData.get(candlestickSeries)) {
          ...
          const precision =
            candlestickSeries.options().priceFormat.precision || 2;
  ...
  const fetchTickerData = async (pair) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `Ticker fetch failed (HTTP ${response.status} from ${url})`
        );
      ...
      const markers = [
        {
          ...
          color: "#2196F3",
          shape: "arrowUp",
          text: "Buy @ " + candleData[20].low.toFixed(precision),
        },
        {
          ...
          color: "#e91e63",
          shape: "arrowDown",
          text: "Sell @ " + candleData[50].high.toFixed(precision),
        },
        {
  ...
  const fetchChartData = async (pair) => {
    ...
    try {
      ...
      console.warn(
        "Server connection failed. Falling back to mock data.",
        error
      );
